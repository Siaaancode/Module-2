...
Above is this latest version of JSLint, tested after making a bunch of changes to the code to harden the localStorage, cleanly separate the Chart.js code, replaced external CDN dependancy, etc.

In the end, I did separate the chart code from the original script.js file, so when testing through JSLint, I did need to test both files at the same time to avoid any errors such as, "Undeclared 'updateChart", etc.
...
- Delete button appears under first input field when page is refreshed. I decided not to fix this bug, as it not a detrimental bug that breaks the application. If I was to fix it, I'd try to remove it, as I wouldn't want the users to intentionally remove the original input field.
...
4. Add options for different currencies. Adding support for multiple currencies would significantly improve the application's usability and accessibility by making it suitable for a wider range of users. Allowing users to select their preferred currency would create a more personalised experience and extend the application's appeal beyond those using a single default currency.
...
- Favicon image, generate by ChatGPT (resized by myself)
...
## Code
### ChatGPT
1.  Used ChatGPT to hide label tags - 
...
2. Fixed duplicate ids issues when new inputs generated
...
3. Add input validation and visible error message for empty or invalid rows
...
4. Harden localStorage/JSON.parse handling with try/catch fallback behaviour
